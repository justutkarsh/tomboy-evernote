...
public class EDAMDemo {
	...
	private static final String authToken = "yourauthtoken";
	...
	public static void main(String args[]) 
		...
		if (demo.intitialize()) {
			...
			for(ImportedNote in:np.getNotesList()){
				try {
					demo.createNote(in);
				}
	...
	private void createNote()
		...
		String fileName = "res/enlogo.png";
	 ...
	 * Create a new note containing a little text and the Evernote icon.
	 */
	private void createNote(ImportedNote inote)
			throws Exception
			{
		// To create a new note, simply create a new Note object and fill in 
		// attributes such as the note's title.
		Note note = new Note();
		note.setTitle(inote.getTitle()!=null?inote.getTitle():"untitled");
		// The content of an Evernote note is represented using Evernote Markup Language
		// (ENML). The full ENML specification can be found in the Evernote API Overview
		// at http://dev.evernote.com/documentation/cloud/chapters/ENML.php
		String content = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
				+ "<!DOCTYPE en-note SYSTEM \"http://xml.evernote.com/pub/enml2.dtd\">"
				+ "<en-note>" 
				+ "<span>"+inote.getText()+"</span><br/>"
				+ "</en-note>";
		note.setContent(content);
		note.addToTagNames(inote.getNotebook()!=null?inote.getNotebook():"untagged");
